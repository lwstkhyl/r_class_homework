...
ggplot(A_pies4) +
 ...
 geom_text(aes(x = rlabel*sin(mid_angle), 
                     ...
                     label = round(relative_abundance,1),
 hjust = hjust, vjust = vjust),nudge_x = 0,
 show.legend = FALSE) +
 coord_fixed() +
...
haircolor_count[[13,1]] <- "NA"
ggplot(haircolor_count, aes(x=fct_reorder(hair_color,count,.desc = F),
                     y=count)) +
  xlab("hair_color") +
  geom_bar(stat = "identity") + 
#stat = "identity"当出现tat_count() can only have an x or y aesthetic.报错时加
  coord_flip()
```
...
res_spread[is.na(res_spread)] <- 0
#将所有的NA替换为0
head(res_spread,n=10)
```
